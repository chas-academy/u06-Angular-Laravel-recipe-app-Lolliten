...
export class AppComponent {
  ...
  onSubmit() {
    ...
    /*if (this.filter.trim() !== '') {
      ...
      // ? const url = `https://api.spoonacular.com/recipes/complexSearch?query=${this.filter}&apiKey=${apiKey}`;

      this.http.get<any>(url).subscribe(
        (data) => {
          this.results = data.results;
          console.log(this.results); // Handle the results?
        },
        (error) => {
    ...
    //Import HTTPClient on top?
  }
    //HTTP request to query for the optins
  }